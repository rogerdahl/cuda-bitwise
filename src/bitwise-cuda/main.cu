...
struct less_than_key3
{
    inline bool operator() (const Base& a, const Base& b)
    {
        for (int i = static_cast<int>(a.program.nOps) - 1; i >= 0; --i) {
            if (i == static_cast<int>(a.program.nOps) - 3) {
...
//        }
        ...
        // This optimization can't be used in the current CUDA version. It causes the threads to fall out of sync.
